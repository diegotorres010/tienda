...
function agregarItem() {
	...
	$('#detalleFactura').append(
		'<tr>'
		+ '<td>' + itemsFactura + '</td>'
		+ '<td> <div class="btn-group"><button class="btn btn-danger" data-toggle="modal" data-target="#"><i class="fa fa-trash"></i></button></td>'
		+ '<td>' + infoProducto.codigoBarras + '</td>'
...
function buscarProducto(idProducto) {
	for (var i = 0; i < productosArray.length; i++) {
		if (productosArray[i].idProducto == idProducto) return productosArray[i];
	}
}
...
$("#nuevoMetodoPago").change(function () {
	...
	if (metodo == "1") {
		...
		$(this).parent().parent().parent().children('.cajasMetodoPago').html(
			...
			'<input type="text" class="form-control" id="nuevoValorCredito" placeholder="000000" required>' +

			'</div>' +

			'</div>')

		$('#nuevoValorCredito').number(true, 2);
